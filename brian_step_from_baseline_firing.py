...
neurons of 5. It has a baseline firing before stepping but can step up AND STAY STEPPED even if I decrease
the noisy input.

THIS ACTUALLY WORKS!!!! WOOOOOOOOOOOOOOOOOOOOO!!!!!!!!!!!!!

* It is quite sensitive to the excitatory connections within the population if they step immediately or randomly within the trial.
* It is sensitive to the number of neurons in the cluster as well. With fewer neurons, the connections must be stronger.
* The input poisson firing is one to one so uncorrelated between neurons. This seemed to be important or the group would pretty
easily step up once all the neurons fire for infrequently but at the same time for low FR.
'''
...
num_neuron_E = 10 # number of excitatory neurons
...
stimulus_tile = np.array([20.0,40.0, 10.])
block_size = 1000.
...
syn_input.connect(j='i') # one to one connections
syn_input.w = '15*mV + rand()*5*mV'
...
syn_excit_conn.connect() # all to all connections
syn_excit_conn.w = '4.5*mV + rand()*1*mV'
syn_excit_conn.delay = '1*ms + rand()*3*ms'
